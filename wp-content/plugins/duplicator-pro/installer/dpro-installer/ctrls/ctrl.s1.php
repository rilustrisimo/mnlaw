...
if ($_POST['archive_engine'] == 'manual') {
	...
	if ($_POST['archive_engine'] == 'shellexec_unzip') {
        ...
        DUPX_Log::info(">>> Starting DupArchive Expand");

        require_once($GLOBALS['CURRENT_ROOT_PATH'].'/lib/dup_archive/classes/util/class.duparchive.util.php');

        $url = "http://$_SERVER[HTTP_HOST]".strtok($_SERVER["REQUEST_URI"], '?');
        $url .= '/lib/dup_archive/daws/daws.php';

        $data = array('action' => 'start_expand', 'archive_filepath' => $archive_path, 'restore_directory' => $GLOBALS['CURRENT_ROOT_PATH']);

        DUPX_Log::info("SPAWNING DAWS WORKER AT $url FOR ACTION $action");

        DupArchiveUtil::postWithoutWait($url, $data);

        // RSR TODO: For now sit in a loop waitinf for daws to complete
