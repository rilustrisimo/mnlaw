	...
	if (bbq_get_current_screen_id() === 'settings_page_bbq_settings') {
		...
		if (!bbq_check_date_expired() && !bbq_dismiss_notice_check()) {
			...
			<div class="notice notice-success">
				<p>
					<strong><?php esc_html_e('Plugin Sale:', 'block-bad-queries'); ?></strong> 
					<?php esc_html_e('Save 20% on any of our', 'block-bad-queries'); ?> 
					...
					<?php esc_html_e('Apply code', 'block-bad-queries'); ?> <code>PLANET2023</code> <?php esc_html_e('at checkout. Sale ends 9/9/23.', 'block-bad-queries'); ?> 
...
function bbq_check_date_expired() {
	...
	$expires = apply_filters('bbq_check_date_expired', '2023-09-09');
