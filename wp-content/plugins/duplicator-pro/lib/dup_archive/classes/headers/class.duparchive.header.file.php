...
//    const MaxStandardHeaderFieldLength = 128;
    ...
    static function createFromFile($filepath, $relativeFilePath)
    {
        ...
        // profile ok - biggest timesink here but no choice
     ...
     * <F><FS>x</FS><MT>x</<MT><FP>x</FP><MD5>x</MD5><RFPL>x</RFPL><RFP>x</RFP></F>
     # F#x#x#x#x#x#x!
     *
     */
...
//        $headerString = "?F#{$this->fileSize}#{$this->mtime}#{$this->permissions}#{$this->md5}#{$this->relativePathLength}#{$this->relativePath}#F!";
        $headerString = "<F><FS>{$this->fileSize}</FS><MT>{$this->mtime}</MT><P>{$this->permissions}</P><MD5>{$this->md5}</MD5><RPL>{$this->relativePathLength}</RPL><RP>{$this->relativePath}</RP></F>";
